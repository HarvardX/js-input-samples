...
//*********************************************************************************
// Replicate underscore.js's _.isEqual() function.
...
//*********************************************************************************
// End of _.isEqual() function.
// Done stealing from underscore.js.
//*********************************************************************************
...
function loadElementsSighted() {
  ...
  for (var i = 0; i < tableArrayLeft.length; i++) {
    label =
      '<tr><td class="spacer-right"></td><td class="label-left openbig">' +
    ...
    if (elementsLeft[tableArrayLeft[i]].type == "image") {
      ...
      content =
        '<tr><td colspan="2" class="content left-image openbig"><img src="' +
    ...
    if (elementsLeft[tableArrayLeft[i]].type == "text") {
      ...
      content =
        '<tr><td colspan="2" class="content left-text openbig">' +
  ...
  for (var i = 0; i < tableArrayRight.length; i++) {
    label =
      '<tr><td class="spacer-right"></td><td class="label-right openbig">' +
    ...
    if (elementsRight[tableArrayRight[i]].type == "image") {
      ...
      $("#hiddenspace").append(
        '<div id="bigelement' +
          ...
          '" alt="' +
          jQuery(
            "<p>" + elementsRight[tableArrayRight[i]].text + "</p>"
          ).text() + // Strip the tags from the text for alt text
          '" /></div>'
      );
    }
  ...
  $(".openbig").on("click tap", function () {
    ...
    var modalWidth = "auto";
    if ($(this).parents("table").hasClass("text")) {
      modalWidth = 650;
    }
    ...
    $("#" + newID).dialog({
      ...
      closeOnEscape: true,
      open: function () {
        // Close that if they click outside the image.
...
function loadElementsScreenReader() {
  ...
  for (var i = 0; i < tableArrayLeft.length; i++) {
    ...
    elementTable.append('<tr id="row' + tableArrayLeft[i] + '"></tr>');
    currentRow = $("#row" + tableArrayLeft[i]);

    if (elementsLeft[tableArrayLeft[i]].type == "text") {
      currentRow.append(
        '<td class="accesstable" id="element' +
          tableArrayLeft[i] +
          '">' +
          elementsLeft[tableArrayLeft[i]].text +
          "</td>"
    ...
    selectionForm =
      '<td class="accesstable"><form><fieldset><legend>' +
      elementsLeft[tableArrayLeft[i]].label +
      "</legend>" +
      selectionForm +
      "</fieldset></form></td>";
...
function handleDrop(event, ui) {
  ...
  for (var i = 0; i < JSProblemState.pairings.length; i++) {
    if (isEqual(JSProblemState.pairings[i], thisPairing)) {
      // Uses underscore.js
      existingMatch = true;
...
function addMatch(indicatorSpace, letter, number) {
  ...
  $("#" + letter + "-" + number).append(
    '<span id="delete' + letter + "-" + number + '" class="delete">[-]</span>'
  );
  $("#delete" + letter + "-" + number).on("click tap", function (event) {
    selfDelete(event);
  });
...
function selfDelete(event) {
  ...
  if (exIndex >= JSProblemState.pairings.length) {
    exIndex = -1;
  }
  if (exIndex >= 0) {
...
function setUpSpace() {
  ...
  if (sighted) {
    ...
    space.html(
      '<div id="leftelements"></div><div id="rightelements"></div><div id="hiddenspace"></div>'
    );
    ...
    instructions.html(
      "[Use checkboxes in the table below to put items into categories.]"
    );
